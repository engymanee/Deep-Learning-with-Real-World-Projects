...
export async function initializeDatabase() {
  ...
  try {
    // Test if tables exist by querying programs table
    ...
    if (error && error.code === 'PGRST116') {
      ...
      const { error: createError } = await supabase.rpc('exec_sql', {
        sql: `
          CREATE TYPE IF NOT EXISTS user_role AS ENUM ('participant', 'team_lead', 'facilitator', 'admin');
          ...
          ALTER TABLE school_teams ADD CONSTRAINT fk_school_teams_program 
            FOREIGN KEY (program_id) REFERENCES programs(id) ON DELETE CASCADE;
          
          ALTER TABLE cohorts ADD CONSTRAINT fk_cohorts_program 
            FOREIGN KEY (program_id) REFERENCES programs(id) ON DELETE CASCADE;
      ...
      if (createError) {
        console.error('[v0] Database initialization error:', createError)
      } else {
